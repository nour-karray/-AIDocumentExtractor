...
def test_public_meta_hides_server_paths_and_hosts(monkeypatch, tmp_path):
    ...
    assert payload["appName"] == "DocIA"
    assert all(item["href"] != "/evaluation" for item in payload["navigation"])


def test_models_endpoint_lists_only_implemented_engines(monkeypatch, tmp_path):
    cfg = load_config(tmp_path)
    monkeypatch.setattr(core, "_docling_available", lambda: False)
    monkeypatch.setattr(core, "_paddleocr_available", lambda: False)
    monkeypatch.setattr(core, "_ollama_available", lambda _host=None: False)
    monkeypatch.setattr(core, "_tesseract_available", lambda _path=None: False)
    with _client(monkeypatch, cfg) as client:
        response = client.get("/api/models")
    assert response.status_code == 200
    model_ids = {item["id"] for item in response.json()["models"]}
    assert model_ids == {"docling-qwen-local", "gemini-api", "ocr-local"}
